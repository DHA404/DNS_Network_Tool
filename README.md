...
- 🚀 **首次运行智能引导** - 自动检测首次运行，提供初始化向导
- ⚙️ **系统自动性能检测** - 自动测试系统最大并发连接数、线程数、DNS解析线程数
- 🔧 **智能参数优化** - 根据检测结果自动计算最优配置（取最大值的2/3）
- 🌍 **DNS服务器自动筛选** - 依次测试所有DNS服务器，自动移除不可用或响应慢的服务器
- 💾 **配置自动保存** - 优化后的配置自动保存到config.json
- 🔄 **重新初始化支持** - 支持随时重新运行初始化向导或重置初始化记录
...
- **首次运行自动提示**：程序首次运行时会自动提示进行初始化
- **系统性能测试**：自动检测系统支持的最大并发连接数、线程数、DNS解析线程数
- **DNS服务器筛选**：测试并筛选可用的DNS服务器
- **智能参数优化**：根据检测结果自动优化配置参数
- **手动运行**：随时从主菜单选择"初始化配置向导"重新运行
